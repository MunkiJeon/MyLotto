...
class MainActivity : AppCompatActivity() {
    ...
    private val results by lazy { findViewById<TextView>(R.id.tv_results) }
    var clickNum = 0
    ...
    private fun initRunButton() {//번호 자동 생성 Function
        runButton.setOnClickListener {
            ...
            saveResults(list.toString())
        }
    }

    private fun saveResults(list: String) {// 로그 생성
        var text: String = results.text.toString()
        ++clickNum
        Log.d("로그태그", "${clickNum}번째: ${list}")
        results.text = "${clickNum}번째: ${list}\n${text}"
    }
